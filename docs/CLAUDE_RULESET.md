...
featured image - Stop Overcomplicating Claude Code: 3 Rules That Actually Work
...
Weird seeing all these complex LLM workflows and tools. Meanwhile I run one continuous Claude convo per project... I've never used a subagent. Never used MCP… And I have wildly good results ¯_(ツ)_/¯— Chris McCord, creator of the Phoenix framework
...
Checking the context usage in a Claude Code session.
Checking the context usage in a Claude Code session.

If you want to see how much of your context window is already used up, you can run the /context slash command in an active Claude Code session. The example above shows that 2.5% of the context window is used up by just one MCP server.
...
Wasp Studio even gives you a visual representation of your full-stack app structure.
...
Example of an llms.txt file structure
Example of an llms.txt file structure

Wasp Claude Code plugin in action.
